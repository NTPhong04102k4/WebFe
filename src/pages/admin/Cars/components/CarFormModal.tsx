...
import { emptyForm, buildCarFormData, type CarFormState } from '../carsHelpers'
...
export function CarFormModal({
  ...
  const { data: carDetail, isLoading: detailLoading } = useQuery({
    ...
    enabled: mode === 'edit' && editingCar !== null,
  })
  ...
  useEffect(() => {
    ...
    reset({
      ...
      importPrice: carDetail.importPrice == null ? '' : String(carDetail.importPrice),
  ...
  }, [mode, carDetail, reset])

  const submit = handleSubmit(async (values) => {
    const fd = buildCarFormData(values, user)
    await onSave(fd)
  })
  ...
  return (
    <Modal
      open
      title={mode === 'create' ? 'Tạo xe' : 'Chỉnh sửa xe'}
      size="lg"
      onClose={onClose}
      footer={
        <div className="flex items-center justify-end gap-2">
          ...
          <button
            form="car-form"
            type="submit"
            disabled={isSaving || (mode === 'edit' && detailLoading)}
          ...
          >
            {isSaving ? 'Đang lưu...' : mode === 'create' ? 'Tạo xe' : 'Lưu thay đổi'}
          </button>
        </div>
      }
    >
      <div className="relative max-h-[65vh] overflow-y-auto">
        ...
        >
          <Input label="Car code" {...register('carCode')} />
          ...
          <Select label="Tình trạng" options={CONDITION_OPTIONS} {...register('condition')} />
          ...
          />
          <Input label="Import price" inputMode="numeric" {...register('importPrice')} />
          <Input label="Sale price" inputMode="numeric" {...register('salePrice')} />

          <Input label="Engine size" inputMode="decimal" {...register('engineSize')} />
          <Input label="Fuel type" {...register('fuelType')} />
          <Input label="Transmission" {...register('transmission')} />
          <Input label="Drive type" {...register('driveType')} />
          ...
          <div className="flex flex-col gap-1 md:col-span-2">
            <label className="block text-sm font-medium text-slate-800">Detailed description</label>
            <textarea
          ...
          <div className="flex flex-col gap-1 md:col-span-2">
            <label className="text-sm font-medium text-slate-800">Ảnh (ImageFiles)</label>
            <input
              type="file"
              accept="image/*"
              multiple
            ...
            {watchedImageFiles.length > 0 && (
              <p className="text-xs text-slate-500">Đã chọn {watchedImageFiles.length} ảnh</p>
            )}
          </div>

          <div className="flex flex-col gap-1 md:col-span-2">
            <label className="text-sm font-medium text-slate-800">Video (VideoFile)</label>
            <input
              type="file"
              accept="video/*"
              className="text-sm"
            ...
            />
            {watchedVideoFile && (
              <p className="text-xs text-slate-500">Đã chọn: {watchedVideoFile.name}</p>
            )}
          </div>
        </form>
      </div>
