...
export function useCarsHandler(): CarsHandlerReturn {
  ...
  const [deleteConfirmCar, setDeleteConfirmCar] =
    useState<CarResponseItem | null>(null);
  ...
  const handleSave = async (fd: FormData) => {
    try {
      if (!editingCarId) {
        ...
        await updateMutation.mutateAsync({ id: editingCarId, fd });
      }
    } catch {
  ...
  return {
    ...
    handlePriceFromChange: (v) => {
      setPage(1);
      setPriceFrom(v);
    },
    handlePriceToChange: (v) => {
      setPage(1);
      setPriceTo(v);
    },
    ...
    handleSave,
    handleDelete: () => {
      if (deleteConfirmCar) deleteMutation.mutate(deleteConfirmCar.carID);
    },
  };
}
