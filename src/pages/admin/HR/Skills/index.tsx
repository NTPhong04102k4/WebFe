...
import { useHrSkillMutations, useHrSkills } from "src/query/hr/useHrQueries";
import type {
  SkillRequest,
  SkillViewModel,
} from "src/services/api/functions/hr/hr.types";
import { SKILL_CATEGORY_OPTIONS } from "@/common/utils/const";
...
export default function AdminHrSkillsPage() {
  ...
  // Danh mục nhóm chuẩn dùng chung với BE: Web/Common/SkillCategories.cs
  const categoryOptions = SKILL_CATEGORY_OPTIONS;
  ...
  const openEdit = (skill: SkillViewModel) => {
    ...
    setForm({
      ...
      category: skill.category || (categoryOptions[0]?.value ?? ""),
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
              Danh mục skill cho kỹ thuật viên
            </p>
          </div>
          <button
      ...
      {error ? (
        ...
        <DataTable
          ...
          columns={columns}
          loading={isLoading}
          getRowId={(row) => String(row.skillID)}
        />
      )}
      ...
      <Modal
        open={open}
        onClose={close}
        title={editing ? "Sửa kỹ năng" : "Tạo kỹ năng"}
        footer={
          <div className="flex justify-end gap-2">
            <button
      ...
      >
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Input
            ...
            onChange={(event) =>
              setForm({ ...form, skillCode: event.target.value })
            }
