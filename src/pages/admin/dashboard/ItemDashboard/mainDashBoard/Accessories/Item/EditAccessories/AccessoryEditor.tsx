...
import { AccessoryFormFields } from "../../EditAccessories";
type Props = {
  open: boolean;
  onClose: () => void;
...
export const AccessoryEditor: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    setForm({
      ...defaultForm,
      accessoryCode: isEditing ? "" : generateAccessoryCode(),
    });
    setImageFile(null);
    setVideoFile(null);
    setError(null);
  }, [open, isEditing, accessoryId, defaultForm, generateAccessoryCode]);

  useEffect(() => {
    if (!open || !isEditing || !accessoryId) return;
    (async () => {
      try {
  ...
  }, [open, isEditing, accessoryId, defaultForm]);
