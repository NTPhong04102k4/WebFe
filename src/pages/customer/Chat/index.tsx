...
export default function CustomerChatPage() {
  ...
  const conversations = useQuery({
    ...
    queryFn: ({ signal }) => chatApi.conversations({ pageSize: 50 }, { signal }),
  ...
  const selectedConversation = conversationList.find((c) => c.conversationID === selectedId);
  ...
  const closeConversation = useMutation({
    mutationFn: (body: CloseConversationRequest) => chatApi.close(selectedId!, body),
    onSuccess: () => {
      ...
      <>
        {parts.map((part, i) =>
          part.toLowerCase() === search.toLowerCase()
            ? <mark key={i} className="bg-yellow-300 text-slate-950 px-0.5 rounded font-semibold">{part}</mark>
            : part
        )}
      </>
    );
      ...
      {showCloseModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            <h3 className="text-lg font-bold text-slate-900 mb-1">Đóng cuộc trò chuyện</h3>
            <p className="text-sm text-slate-500 mb-4">Hãy để lại đánh giá để giúp chúng tôi cải thiện dịch vụ.</p>

            <div className="mb-4">
              <label className="block text-sm font-medium text-slate-700 mb-2">Đánh giá chất lượng hỗ trợ</label>
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} onClick={() => setCloseRating(star)} className="focus:outline-none">
                    <Star className={`h-8 w-8 transition-colors ${
                      star <= closeRating ? "fill-amber-400 text-amber-400" : "text-slate-300 hover:text-amber-300"
                    }`} />
                  </button>
                ))}
              </div>
      ...
      <div className="flex flex-col md:grid flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm md:grid-cols-[340px_1fr]">
        ...
        <div className="flex flex-col border-b md:border-b-0 md:border-r border-slate-200 overflow-hidden h-[25vh] md:h-full shrink-0">
          ...
          {conversations.isLoading && (
            <div className="p-4 text-center text-sm text-slate-450">Đang tải...</div>
          )}
                ...
                return (
                  <button
                    ...
                    className={`relative block w-full px-4 py-3.5 text-left transition-all hover:bg-slate-50 border-b border-slate-100 ${
                      isSelected ? "bg-blue-50/70 border-l-4 border-blue-600 pl-3" : "pl-4"
                    }`}
          ...
          <div className="border-t border-slate-200 bg-slate-50/50 p-4 shrink-0">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">Tạo hội thoại mới</h3>
        ...
        <div className="flex overflow-hidden flex-1 min-h-0 relative">
          {selectedId ? (
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden relative">
                ...
                <div className="flex items-center justify-between border-b border-slate-200 bg-amber-50/80 px-6 py-2 shrink-0 gap-4">
                  ...
                  >
                    ...
                    <span className="truncate">
                      Tin nhắn ghim:{" "}
                      <span className="font-normal text-slate-700">&ldquo;{pinnedMessage.content}&rdquo;</span>
                    </span>
                  </div>
                </div>
              )}
              ...
              {messages.length === 0 && (
                <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-6 bg-slate-50/20">
                  ...
                  <p className="text-sm">Bắt đầu trò chuyện bằng cách nhập tin nhắn bên dưới.</p>
                </div>
              )}
              {messages.length > 0 && (
                <FlatList
                  ...
                  renderItem={({ item }) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex flex-col relative group ${mine ? "items-end" : "items-start"} rounded-lg p-1`}
                      >
              ...
              <div className="border-t border-slate-100 p-4 bg-white shrink-0">
                {isClosed ? (
                  <div className="flex items-center gap-2 rounded-lg bg-amber-50 border border-amber-200 p-3 text-amber-800 text-sm">
                    ...
                    <span>Cuộc hội thoại này đã đóng. Nếu cần hỗ trợ thêm, vui lòng tạo hội thoại mới.</span>
                  </div>
                ) : (
            ...
            <div className="flex flex-1 flex-col items-center justify-center p-8 text-slate-440 text-center">
              ...
              <h3 className="font-semibold text-slate-700 text-base mb-1">Hỗ trợ trực tuyến SoldCars</h3>
              <p className="text-sm max-w-sm">
                Chọn một cuộc trò chuyện từ danh sách bên trái hoặc tạo mới để trao đổi với đội ngũ chăm sóc khách hàng.
              </p>
            </div>
          )}
        </div>
