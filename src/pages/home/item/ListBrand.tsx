 ...
 if(detail===false){
  return (
    <>
      ...
      <div className="flex flex-row flex-wrap items-center justify-center gap-2 mt-16 pr-[45px] pl-[45px] ">
        {DATA.map((item, index) => {
          return (
            <div onClick={()=>handleBody(item)} className="flex flex-col gap-2 hover:scale-105 active:scale-90 duration-500 flex-1" key={item.id}>
  ...
  return (
    ...
    <h2 className="text-3xl 2xl:4xl font-semibold font-sans self-center  mt-16 ">
      ...
      <h2 className="text-base font-sans font-normal text-[#000] cursor-pointer hover:underline active:opacity-70  ">Show All Brands</h2>
      <GoArrowUpRight size={24} color="#000" className="cursor-pointer active:opacity-70"/>
      </div>
    </div>
      ...
      <div className="inline-flex  flex-shrink-0 items-center justify-center gap-2 mt-8 w-[90%] self-center overflow-hidden">
        {DATA.map((item, index) => {
          if(index<6){
            return (
              <div  className="flex flex-col p-4 border-[0.5px] rounded-lg border-gray-200 " style={{width:`calc(16.7% - 8px`}} key={item.id}>
                <div className="w-1/2 self-center h-auto aspect-square bg-center bg-cover " style={{backgroundImage:`url(${item.img})`}}/>
...
const ItemCarBrand = styled.div<{ srcImage: string }>`
  ...
  align-items: center;
  background-image: url(${(props) =>props.srcImage || "src/assets/images/homepage/cars.png"});
